...
class RetroArchDisconnectError(nDSException):
    pass


class InvalidEmulatorStateError(nDSException):
    pass


class BadRetroArchResponse(nDSException):
    pass
...
class LinksAwakeningClient():
    ...
    async def wait_for_retroarch_connection(self):
        ...
        self.nds = RAnDS(self.retroarch_address, self.retroarch_port)

        while True:
            try:
                version = await self.nds.get_retroarch_version()
                NO_CONTENT = b"GET_STATUS CONTENTLESS"
                status = NO_CONTENT
                core_type = None
                N_DS = b"n_ds"
                while status == NO_CONTENT or core_type != N_DS:
                    status = await self.nds.get_retroarch_status()
                    if status.count(b" ") < 2:
                    ...
                    if core_type != N_DS:
                        logger.info(
                            f"Core type should be '{N_DS}', found {core_type} instead - wrong type of ROM?")
                        await asyncio.sleep(1.0)
                        continue
    ...
    async def reset_auth(self):
        auth = binascii.hexlify(await self.nds.async_read_memory(0x0134, 12)).decode()
        self.auth = auth

    async def wait_and_init_tracker(self):
        await self.wait_for_game_ready()
        self.tracker = LocationTracker(self.nds)
        self.item_tracker = ItemTracker(self.nds)
        self.gps_tracker = GpsTracker(self.nds)
    ...
    async def recved_item_from_ap(self, item_id, from_player, next_index):
        ...
        status = (await self.nds.async_read_memory_safe(LAClientConstants.wLinkStatusBits))[0]
        ...
        next_index += 1
        self.nds.write_memory(LAClientConstants.wLinkGiveItem, [
                                  item_id, from_player])
        status |= 1
        status = self.nds.write_memory(LAClientConstants.wLinkStatusBits, [status])
        self.nds.write_memory(LAClientConstants.wRecvIndex, struct.pack(">H", next_index))

    should_reset_auth = False
    async def wait_for_game_ready(self):
        logger.info("Waiting on game to be in valid state...")
        while not await self.nds.check_safe_gameplay(throw=False):
            if self.should_reset_auth:
                self.should_reset_auth = False
                raise nDSException("Resetting due to wrong archipelago server")
        logger.info("Game connection ready!")

    async def is_victory(self):
        return (await self.nds.read_memory_cache([LAClientConstants.wGameplayType]))[LAClientConstants.wGameplayType] == 1
    ...
    async def main_tick(self, item_get_cb, win_cb, deathlink_cb):
        ...
        current_health = (await self.nds.read_memory_cache([LAClientConstants.wLinkHealth]))[LAClientConstants.wLinkHealth]
        ...
        if self.pending_deathlink:
            ...
            self.nds.write_memory(LAClientConstants.wLinkHealth, [0])
        ...
        recv_index = struct.unpack(">H", await self.nds.async_read_memory(LAClientConstants.wRecvIndex, 2))[0]
            ...
            try:
                ...
                while True:
                    ...
                    if self.client.should_reset_auth:
                        ...
                        raise nDSException("Resetting due to wrong archipelago server")
            except (nDSException, asyncio.TimeoutError, TimeoutError, ConnectionResetError):
                await asyncio.sleep(1.0)
